	...
	public async Task AddHra(Add_Action action, IDispatcher dispatcher)
	{
		...
		try
		{
			...
			if (sprocTuple.Item2 != 2601) // Unique Index Violation
			{
				...
				dispatcher.Dispatch(new Response_Message_Action(ResponseMessage.Info, $"{LivingMessiah.Web.Pages.Sukkot.ManageRegistration.Constants.Effects.RepopulateMessage}"));
			}
			else
			{
		...
		catch (Exception ex)
		{
			...
			dispatcher.Dispatch(new Response_Message_Action(ResponseMessage.Failure, $"{LivingMessiah.Web.Pages.Sukkot.ManageRegistration.Constants.Effects.ResponseMessageFailure}."));
		}
	}
