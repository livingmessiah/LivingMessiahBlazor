...
// 1. Action
public record Add_Action(Donations.FormVM FormVM);
public record Set_Donation_FormVM_Action(Donations.FormVM? FormVM);  // ToDo rename to ???
...
public record DonationState
{
	...
	ToDo: It seems to me that it's not the job of DonationState to set this,
	but should be the Job of IndexState e.g.

```csharp
[ReducerMethod]	public static IndexState On_Set_VisibleComponent(IndexState state, Set_VisibleComponent_Action action)
