...
public partial class DatabaseSelectorButton
{
	...
	[Parameter] public EventCallback<Enums.Database> OnDatabaseSelected { get; set; }

	readonly string inside = $"page {Page.Index}; class: {nameof(DatabaseSelectorButton)}";

	private void OnButtonClicked(Enums.Database database)
	{
	...
	public string ActiveFilter(Enums.Database filter)
	{
